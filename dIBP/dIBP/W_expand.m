function result = W_expand(new_k, dim, Z, U, V, W, nuep, sigma_w)
...
% W = W_true; U = U_true; V = V_true; % with new feature in sampleUV.m 
	...
	if dim == 2
		...
		for d = 1:new_k
			for l = 1:L
				l_curr = log(normpdf(W(K + d, l), 0, sigma_w)) + Z_UVWlogpdf(Z, U, V, W, nuep);
				w_prop = normrnd(W(K + d, l), sigma_w); % simply use normal proposal
				W(K + d, l) = w_prop;
				l_prop = log(normpdf(W(K + d, l), 0, sigma_w)) + Z_UVWlogpdf(Z, U, V, W, nuep);
	...
	elseif dim == 1
		...
		for d = 1:new_k
			for k = 1:K
				l_curr = normpdf(W(k, L + d), 0, sigma_w) * exp(Z_UVWlogpdf(Z, U, V, W, nuep));
				w_prop = normrnd(W(k, L + d), sigma_w);
				W(k, L + 1) = w_prop;
				l_prop = normpdf(W(k, L + d), 0, sigma_w) * exp(Z_UVWlogpdf(Z, U, V, W, nuep));
				acc_w = exp(min(0, l_prop - l_curr));
